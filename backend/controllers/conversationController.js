...
const getConversationPatient = async (req, res) => {
    try {
        ...
        if (!conversation && !conversation2) {
            ...
            return res.status(200).json({ message: 'No conversation was found. A new one was created successfully', conversation: newConversation });
        }
        //get all messages in the conversation
        if (conversation) {
            const messages = await Message.find({ conversationId: conversation._id });
            return res.status(200).json({ message: 'Conversation retrieved successfully', conversation, messages });
        }
        else {
...
module.exports = { getConversationPatient, getConversationPharmacist, sendMessagePharmacist, getMessages };
